...
from tools.glamm_eval_utils import IGNORE_INDEX, IMAGE_TOKEN_INDEX
...
class LlavaMetaModel:
    def __init__(self, config):
        ...
        self.region_encoder = MLVLROIQueryModule(
            embed_dims=1024, out_dims=4096, num_levels=4
        )
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            if self.config.with_region:
                ...
                mlvl_reg_features = image_forward_outs.hidden_states[
                ...
                mlvl_reg_features = [
                    item[:, 1:].to(images.dtype) for item in mlvl_reg_features
                ]

                if bboxes is not None and (len(bboxes) > 0):
                    mlvl_reg_features = self.model.region_encoder(
                        mlvl_reg_features, bboxes
                    )
                else:
        ...
        cur_image_idx = 0
        for batch_idx, (cur_input_ids, reg_feat) in enumerate(
            zip(input_ids, mlvl_reg_features)
        ):  # Adjusted the loop to include reg_feat
            curr_full_input_ids = []
            ...
            while image_token_indices.numel() > 0:
                ...
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    ...
                    curr_full_input_ids.append(
                        cur_input_ids[image_token_start - 1 : image_token_start]
                    )
                    curr_full_image_token = torch.full(
            ...
            curr_full_input_ids = [
                x.to(device=self.device) for x in curr_full_input_ids
            ]
            ...
            if reg_feat is not None:
                ...
                cur_new_input_embeds = (
                    cur_new_input_embeds
                    * (~reg_mask).to(cur_new_input_embeds.dtype)[:, None]
                    + reg_embeds
                )
