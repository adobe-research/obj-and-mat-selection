...
try:
    ...
    class BaseRoIExtractor(nn.Module):  # type: ignore[no-redef]
        def __init__(self, *a, **kw):
            nn.Module.__init__(self)
        def forward(self, *a, **kw):
            raise ImportError(
                "mmdet is not installed. The region encoder is training-only "
                "and not used in the MAOAM demo."
            )
...
def str2reg(input_str):
    ...
    for match in matches:
        results.append(
            [float(match[0]), float(match[1]), float(match[2]), float(match[3])]
        )
    return results
...
def padding_to(inputs, max=300):
    ...
    if inputs.dim() > 1:
        padding = inputs.new_zeros(num_padding, *inputs.size()[1:], dtype=inputs.dtype)
    else:
...
class MLVLFuseModule(nn.Module):
    ...
    def _init_layers(self):
        self.input_conv = nn.ModuleList(
            [
                nn.Conv2d(self.input_dims + 2, self.embed_dims, 1)
    ...
    def _single_shuffle(self, inputs, conv_module):
        ...
        for single_conv_m in conv_module:
            ...
            for fuse_lvl_tuple in self.fuse_lvl_list:
                ...
                from_top = F.interpolate(
                    from_top.to(torch.float32),
                    size=tar_input.shape[-2:],
                    mode="bilinear",
                    align_corners=True,
                )
                from_down = down_input[:, self.remain_chs :][:, : self.shuffle_channles]
                from_down = F.interpolate(
                    from_down.to(torch.float32),
                    size=tar_input.shape[-2:],
                    mode="bilinear",
                    align_corners=True,
                )
                fused_inputs.append(
                    torch.cat(
                        [remain, from_top.to(remain.dtype), from_down.to(remain.dtype)],
                        dim=1,
                    )
                )
...
class MLVLROIQueryModule(nn.Module):
    ...
    def forward(self, mlvl_feats, bboxes):
        ...
        if mlvl_feats[0].dim() == 3:
            ...
            mlvl_feats = [
                item.reshape(b, h, w, c).permute(0, 3, 1, 2) for item in mlvl_feats
            ]
        base_shape = mlvl_feats[0].shape[-2:]
        num_level = len(mlvl_feats)
        to_shape = [
            (base_shape[0] * 2**level, base_shape[1] * 2**level)
...
class MlvlRoIExtractor(BaseRoIExtractor):
    def __init__(
        ...
        super(MlvlRoIExtractor, self).__init__(
    ...
    def forward(self, feats, rois, roi_scale_factor=None):
        ...
        roi_feats = feats[0].new_zeros(
            self.fuse_level, rois.size(0), self.out_channels, *out_size
        )
        ...
        for i in range(num_levels):
            if len(rois) > 0:
                ...
                roi_feats_t = self.roi_layers[i](
                    feats[i].to(torch.float32), rois_.to(torch.float32)
                )
