...
class Qwen2_5_VL(BaseModel):
    ...
    def forward(self,
        ...
        pixel_values = torch.cat(pixel_values, dim=0).to(device=device, dtype=torch.bfloat16)
        image_grid_thw = data['image_grid_thw']
        image_grid_thw = torch.cat(image_grid_thw, dim=0).to(device=device)
        
        
        if data["input_ids"].is_cuda:
            if pixel_values.dtype not in (torch.float16, torch.bfloat16):
                raise RuntimeError(f"pixel_values must be fp16/bf16 for flash-attn, got {pixel_values.dtype}")
