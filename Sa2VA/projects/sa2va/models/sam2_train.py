...
class SAM2TrainRunner(BaseModule):
    def __init__(
        ...
        self._warned_resize_to_1024 = False

    def preprocess_image(self, image: torch.Tensor) -> torch.Tensor:
        if tuple(image.shape[-2:]) != (1024, 1024):
            if not self._warned_resize_to_1024:
                print(
                    f"[SAM2TrainRunner] WARNING: resizing grounding image from {tuple(image.shape[-2:])} to (1024, 1024). "
                    "Temporary workaround; fix upstream preprocessing.",
                    flush=True,
                )
                self._warned_resize_to_1024 = True
            image = F.interpolate(
                image.unsqueeze(0).to(torch.float32),
                size=(1024, 1024),
                mode="bilinear",
                align_corners=False,
            ).squeeze(0).to(dtype=image.dtype)
